...
<link href="${pageScope.webRoot}static/styles/bootstrap-3.3.7-dist/css/bootstrap.min.css" rel="stylesheet">
<script src="${pageScope.webRoot}static/styles/bootstrap-3.3.7-dist/js/bootstrap.min.js"></script>
...
var totalRecord,currentPage;//总记录数和当前所在页
//1、页面加载完成以后，直接去发送ajax请求,要到分页数据
$(function(){
	getDepts("#emps_query select");
	//去首页
	to_page(1);
});
...
function build_page_info(result){
	...
	$("#page_info_area").append("当前"+currentPage+"页,总"+totalPage+"页,总"+totalRecord+"条记录");
}
...
//1、查出所有的部门信息并显示在下拉列表中
function getDepts(ele){
	//清空之前下拉列表的值
	$(ele).empty();
	$.ajax({
		url:"${pageScope.webRoot}depts",
		type:"GET",
		success:function(result){
			// {"code":800200,"msg":"处理成功！","extend":{"depts":[{"deptId":1,"deptName":"开发部"},{"deptId":2,"deptName":"测试部"}]}}
			//显示部门信息在下拉列表中
			$("<option></option>").append("请选择").attr("value",-1).appendTo(ele);
...
$("#emp_update_btn").click(function(){
	...
	//2、发送ajax请求保存更新的员工数据
	$.ajax({
		url:"${pageScope.webRoot}emp/"+$(this).attr("edit-id"),
		type:"PUT",
...
$(document).on("click",".delete_btn",function(){
	...
	if(confirm("确认删除【"+empName+"】吗？")){
		//确认，发送ajax请求删除即可
		$.ajax({
			url:"${pageScope.webRoot}emp/"+empId,
			type:"DELETE",
			success:function(result){
				alert(result.msg);
				//刷新本页
...
$("#check_all").click(function(){
	//注意：attr获取自定义属性的值；prop修改和读取dom原生属性的值。
	//我们这些dom原生的属性,attr获取checked是undefined;
...
$("#emp_delete_all_btn").click(function(){
	...
	if(confirm("确认删除【"+empNames+"】吗？")){
		//发送ajax请求删除
		$.ajax({
			url:"${pageScope.webRoot}emp/"+del_idstr,
			type:"DELETE",
			success:function(result){
				alert(result.msg);
				//回到当前页面
